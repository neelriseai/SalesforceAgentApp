...
try {
    ...
    $planPath='data/multi-module-plan.json'
    $plan=Get-Content $planPath -Raw | ConvertFrom-Json
    if ($plan.dataset -ne 'SDA-CROSS-MODULE-v1' -or $plan.expectedBusinessRows -ne 368 -or $plan.currency -ne 'USD') {throw 'Unexpected synthetic data plan.'}
    ...
    $templatePath=Join-Path $PSScriptRoot 'multi-module.apex.template'
    $request=(Get-Content $templatePath -Raw).Replace('__PLAN_BASE64__',$payload).Replace('__MODE__',$Mode).Replace('__ORG_FINGERPRINT__',$fingerprint)
    ...
    $folder=Join-Path $projectRoot ('artifacts/multi-module/'+$runId)
    ...
    $report=[ordered]@{runId=$runId;endedAtUtc=(Get-Date).ToUniversalTime().ToString('o');planSha256=(Get-FileHash $planPath -Algorithm SHA256).Hash.ToLowerInvariant();runnerSha256=(Get-FileHash $templatePath -Algorithm SHA256).Hash.ToLowerInvariant();result=$result}
